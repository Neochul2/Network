...
이번 장부터는 Transport Layer의 대표 프로토콜인 TCP를 중심으로 내부 구조를 학습한다.
...
이 제어 정보가 저장되는 부분을 TCP Header라고 한다.

TCP Header가 추가된 하나의 전송 단위를 TCP Segment라고 한다.
...
TCP는 데이터를 Segment라는 전송 단위로 나누어 전송한다.
...
TCP는 Payload를 단순한 Byte Stream으로 취급하여 전송할 뿐이다.
...
TCP Header는 최소 20Byte, 최대 60Byte까지 사용할 수 있다.
...
Header는 데이터를 설명하는 영역이고,

Payload는 실제 데이터가 저장되는 영역이다.
